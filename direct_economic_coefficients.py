...
Consumes normalized authoritative source observations and produces a long-form,
versioned country x statistical-sector x year x outcome coefficient layer.

The calculation layer is supplier-agnostic: it has no canonical-entity,
social-economy, directory or client publication dependency.
"""
...
COEFFICIENT_FIELDS = [
    ...
    "coefficient_unit",
    "denominator_method",
    "denominator_compatibility_class",
    "source_family",
    "source_organisation",
    "source_dataset_ids",
...
QA_FIELDS = [
    ...
    "missing_denominator_groups",
    "nonpositive_denominator_groups",
    "missing_numerator_records",
    "accounting_identity_checks",
    "accounting_identity_failures",
    "trade_route_groups",
    "output_route_groups",
]
...
def _route_for_sector(sector_code: str, config: Mapping[str, Any]) -> tuple[str, Mapping[str, Any]]:
    ...
    for route_name, route in routes.items():
        if route_name == "default":
            continue
        sectors = {clean(x) for x in route.get("model_sector_codes", [])}
...
def normalize_source_rows(rows: list[dict[str, str]], *, config: Mapping[str, Any]) -> tuple[list[dict[str, str]], dict[str, int]]:
    ...
    qa["duplicate_source_rows"] = duplicate_count
    qa["conflicting_source_rows"] = conflict_count
...
def _coefficient_record(*, group_key, rows, route, outcome_code, outcome, denominator, numerator, config, generated_at, qa_status, qa_reason):
    country, sector_code, year = group_key
    source_rows = [row for row in (denominator, numerator) if row is not None]
    ...
    if qa_status == "calculated":
        result = dec(numerator["value"], "numerator") / dec(denominator["value"], "denominator")
...
def build_coefficients(source_rows: list[dict[str, str]], *, config: Mapping[str, Any], generated_at: str) -> dict[str, Any]:
    ...
    calculation_keys = sorted({(r["country"], r["model_sector_code"], r["reference_year"]) for r in normalized_rows})
    ...
    for group_key in calculation_keys:
        ...
        fallback_rows = [r for r in normalized_rows if (r["country"], r["model_sector_code"], r["reference_year"]) == group_key]
        ...
        held_count = 0
        for outcome_code, outcome in config["outcomes"].items():
            numerator = _select_concept(family_rows, outcome["numerator_concept_code"]) if family_rows else None
            ...
            coefficient_rows.append(_coefficient_record(
                ...
                numerator=numerator, config=config, generated_at=generated_at,
                qa_status=status, qa_reason=reason,
            ))
